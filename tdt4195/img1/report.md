...
Convolution is an linear operation whereas correlation measures the similarity of two signals. Symmetric kernels such as the Gaussian kernel will yield the same results for convolution and correlaion because they are symmetric. 

#### 5: When and why is padding necessary when performing 2D convolution? Give a couple of examples of commonly used padding types.
It may be necessary to use padding to prevent circular convolution with different lengths of signals when doing FFT. 

Zero-padding is a very common type of padding, but also one can also use repeat elements or use a mirror reflection of the source.
